...
def execute_command(command):
    ...
    if "open youtube" in command:
        ...
        jarvis_speak("Opening Spotify")
        webbrowser.open("https://open.spotify.com")  # Opens the Spotify web player
    elif "play" in command and "on spotify" in command:
        song_name = command.split("play")[-1].strip()  # Get the song name after "play"
        jarvis_speak(f"Playing {song_name} on Spotify")
        search_query = song_name.replace(" ", "%20")  # Encode spaces for the URL
        webbrowser.open(f"https://open.spotify.com/search/{search_query}")  # Open search results for the song
